...
function StudentViewCourseProgressPage() {
    ...
    async function updateCourseProgress() {
        if (currentLecture) {
            const response = await markLectureAsViewedService(auth?.user?._id,
    ...
    useEffect(() => {
        if (showConfetti) setTimeout(() => setShowConfetti(false), 7000);
    }, [showConfetti]);
    ...
    return (
        <div className="flex flex-col h-screen bg-[#1c1d1f] text-white" >
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div
                    className={`fixed top-16 right-0 bottom-0 h-[calc(100vh-4rem)] w-[400px] bg-[#1c1d1f] border-l border-gray-700 transition-all duration-300 z-20 ${isSideBarOpen ? "translate-x-0" : "translate-x-full"
                        }`}
                >
                    ...
                    <Tabs defaultValue="content" className="h-full flex flex-col">
                        <TabsList className="grid bg-[#1c1d1f] w-full grid-cols-2 p-0 h-14">
                            <TabsTrigger
                                value="content"
                                className=" text-white rounded-none h-full data-[state=active]:bg-white"
                            >
                                Course Content
                            </TabsTrigger>
                            <TabsTrigger
